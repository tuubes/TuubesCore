...
object EnsureInGroup {
	def impl(c: whitebox.Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
		...
		def wrapMethodContent(method: DefDef) = {
			val q"$mods def $name[..$t](...$paramss)(implicit ..$iParams): $result = $expr" = method
			...
			q"""$mods def $name[..$t](...$paramss)(implicit ..$newImplicitParams): $safeResult = {
				if($iParam eq execGroup) {
					...
					execGroup.execute(() => $name(...${normalParams.map(_.map(_.name))})(..${
				newImplicitParams.
				map(v => if (iParam.decodedName.toString == v.name.decodedName.toString) iParam
		...
		def addImplicitParam(implicitParams: Seq[ValDef]): (Seq[ValDef], TermName) = {
			...
			val intParam = implicitParams.find(_.tpt.asInstanceOf[Ident].name == execGroupType)
			intParam match {
				case Some(valDef) => (implicitParams, valDef.name)
				case None =>
					val newParams = implicitParams :+ q"implicit val callerGroup: $execGroupType".asInstanceOf[ValDef]
		...
		annottees.map(_.tree) match {
			...
			case _ => c.abort(c.enclosingPosition, "@EnsureInGroup can only be used with methods")
		}
	}
}