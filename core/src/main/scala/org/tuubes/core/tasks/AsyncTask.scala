...
abstract class AsyncTask[A, B](private[this] val function: A => Try[B],
  ...
  /**
	 ...
	 */
  @varargs
  def thenIOFile[C](file: File,
                    f: ThrowableFunction2[B, FileChannel, C],
  ...
  /**
	 ...
	 */
  private def ioFunctionFile[R](file: File,
                                f: (B, FileChannel) => R,
                                openOptions: OpenOption*): B => R = { (b: B) =>
    {
  ...
  /**
	 ...
	 */
  private def ioFunctionURL[R](url: String, f: (B, URLConnection) => R): B => R = { (b: B) =>
    {
...
object AsyncTask {
  ...
  private def wrap[T, R](f: ThrowableFunction1[T, R]): T => Try[R] = { t =>
    Try(f(t))
  }
  ...
  /** Wraps a function in a Try to catch all the non-fatal exceptions */
  private def wrap[T, R](f: (T) => Try[R]): T => Try[R] = { t =>
    Try(f(t)).flatten
  }
  ...
  /**
	 ...
	 */
  @varargs
  def awaitSome[R, T <: R, A](count: Int,
                              tasks: AsyncTask[A, T]*): AsyncTask[Try[VArr[R]], VArr[R]] = {
  ...
  @tailrec
  private[tasks] def markRootSubmitted(tasks: TraversableOnce[AsyncTask[_, _]]): Unit = {
  ...
  private[tasks] def successConvergenceN[A](action: AsyncTask[Try[VArr[A]], _ <: Any],
                                            count: Int): A => Unit = {
  ...
  private[tasks] def successConvergence1[A](action: AsyncTask[Try[A], A]): A => Unit = {
  ...
  private[tasks] def failureConvergenceN[A](action: AsyncTask[Try[VArr[A]], _ <: Any],
  ...
  private[tasks] def failureConvergence1[A](action: AsyncTask[Try[A], A],
                                            failureTolerance: Int): (Any, Throwable) => Unit = {
    // Counts the number of tolered failures before the `action` fails
    val toleranceCounter = new AtomicInteger(failureTolerance)
    // This function is called by each failed task that we wait for. Its goal is to call the
