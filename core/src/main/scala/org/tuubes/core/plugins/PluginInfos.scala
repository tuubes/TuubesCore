...
object PluginInfos {
  def inspect(file: File): Try[PluginInfos] = {
    val url = file.url
    val classLoader =
      new OpenURLClassLoader(url, classOf[PluginInfos].getClassLoader)
    val pluginClass: Try[Class[_ <: Plugin]] =
      loadPluginClass(file, classLoader)
      ...
      case e: Exception =>
        throw new PluginLoadingException(
          "Unable to load the plugin description, please check that the plugin's main" +
            "class has a companion object extending PluginDescription",
          e)
    }
  }
