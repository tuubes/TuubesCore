package org.tuubes.server

import better.files.File
import org.fusesource.jansi.AnsiConsole
import org.tuubes.GameRegistry
import org.tuubes.command.{CommandSystem, StopCommand}
import org.tuubes.entity.mobs.Player
import org.tuubes.network.ProtocolLibAdapter
import org.tuubes.plugin._
import org.tuubes.world.{World, WorldType}
...
object PhotonServer {
	// Logger
	private[tuubes] val logger = LoggerFactory.getLogger("PhotonServer")
	...
	private[tuubes] def registerWorld(w: World): Unit = worldsNameMap.put(w.name, w)
	private[tuubes] def unregisterWorld(w: World): Unit = worldsNameMap.remove(w.name)
	...
	private[tuubes] def shutdown(): Unit = {
		// TODO
