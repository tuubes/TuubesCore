...
import org.tuubes.entity.{MobType, ObjectType}
import org.tuubes.item.ItemType
import org.tuubes.server.PhotonServer
import org.tuubes.server.PhotonServer.DirConfig
import org.tuubes.world.BiomeType
...
object GameRegistry {
	...
	// Get ID by name
	private[tuubes] def blockId(name: String): Int = blocksConfig.get[Config](name).get("id")
	private[tuubes] def entityMobId(name: String): Int = mobsConfig.get(name)
	private[tuubes] def entityObjectId(name: String): Int = objectsConfig.get(name)
	private[tuubes] def itemId(name: String): Int = itemsConfig.get[Config](name).get("id")
	private[tuubes] def biomeId(name: String): Int = biomesConfig.get(name)

	// Get type by ID (and additional data if needed)
	private[tuubes] def block(id: Int, metadata: Int): BlockType = blocksIdMap.get(id).variant(metadata)
	private[tuubes] def entityMob(id: Int): MobType = mobsIdMap.get(id)
	private[tuubes] def entityObject(id: Int): ObjectType = objectsIdMap.get(id)
	private[tuubes] def item(id: Int, damageData: Int): ItemType = itemsIdMap.get(id).variant(damageData)
	private[tuubes] def biome(id: Int): BiomeType = biomesIdMap.get(id)

	// Get registration info by ID
	private[tuubes] def blocks(id: Int): Registration[BlockType] = blocksIdMap.get(id)
	private[tuubes] def items(id: Int): Registration[ItemType] = itemsIdMap.get(id)
	private[tuubes] def block(fullId: Int): BlockType = block(fullId >> 4, fullId & 16)
	 ...
	 */
	private[tuubes] sealed trait Registration[A <: Type[_]] {
	 ...
	 */
	private[tuubes] final class VariantsRegistration[A <: Type[_]](firstVariantType: A,
																   firstVariantData: Int) extends Registration[A] {
		private[tuubes] val variants = new ConcurrentIndexMap[A]
	 ...
	 */
	private[tuubes] final class SingleRegistration[A <: Type[_]](val `type`: A) extends Registration[A] {
		/** Returns the basic type. The data doesn't matter*/
		override def variant(data: Int): A = `type`
	}

	// --- Standard Types Registration ---
	private[tuubes] def autoRegister(): Unit = {
