...
final class Rotation3(y: Float, private[this] val p: Float, r: Float) {
  ...
  def +(a: Rotation3) =
    new Rotation3(yaw + a.yaw, pitch + a.pitch, roll + a.roll)
  ...
  def toDegrees: (Float, Float, Float) =
    (deg(yaw).toFloat, deg(pitch).toFloat, deg(roll).toFloat)
}

object Rotation3 {

  /** Implicit conversion from Rotation3 to Rotation2 by removing the roll */
  implicit def toRotation2(r3: Rotation3): Rotation2 =
    new Rotation2(r3.yaw, r3.pitch)
