...
object PluginMain {
  ...
  def impl(c: whitebox.Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    def createResult(cls: ClassDef, optionalCompanion: Option[ModuleDef]): c.Expr[Any] = {
      val q"class ${clsName: TypeName} extends {..$e} with ..$clsParents {..$clsBody}" =
        cls
      val (newClsBody, getters) =
        extractValues(clsName, clsBody.asInstanceOf[Seq[Tree]], Values: _*)
      ...
      val companion = optionalCompanion.map { comp =>
        val q"object $obj extends {..$e} with ..$parents { ..$companionBody }" =
          comp
      ...
      val newCls =
        q"class $clsName extends {..$e} with ..$clsParents {..$newClsBody}"
    ...
    def extractValues(clsName: TypeName,
                      clsBody: Seq[Tree],
                      vals: String*): (Seq[Tree], Seq[Tree]) = {
    ...
    annottees.map(_.tree) match {
      ...
      case (cls: ClassDef) :: (comp: ModuleDef) :: Nil =>
        createResult(cls, Some(comp))
      case _ =>
        c.abort(c.enclosingPosition, "@PluginMain can only be used with classes")
    }
  }
}
